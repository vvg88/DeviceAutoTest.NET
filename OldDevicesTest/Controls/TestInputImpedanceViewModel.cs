...
{
    public class TestInputImpedanceViewModel : ReadDataViewModel
    {
        public TestInputImpedanceViewModel(ScriptEnvironment environment, NeuroMEPMicro device, NeuroMEPMicroAmplifierState deviceAmplifierState)
            : base(environment, device/*, deviceAmplifierState*/)
        {
            for (int i = 0; i < ChannelsCount; i++)
            {
                Rows.Add(new InputResistanceInfo(i, 3));
                //int maxRange = ChannelsInfo[i].ChannelSupportedRanges.Count - 1;
                if (ChannelsInfo[i].ChannelSupportedRanges.Count > 0)
                    ChannelsInfo[i].SelectedChannelRange = ChannelsInfo[i].ChannelSupportedRanges[ChannelsInfo[i].ChannelSupportedRanges.Count - 1];
                if (ChannelsInfo[i].LowFreqPassBands.Count > 0)
                    ChannelsInfo[i].SelectedLowFreq = ChannelsInfo[i].LowFreqPassBands[0];
            }
        }
        ...
        private void SwitchFreq(bool switchTo2Hz)
        {
            ...
            TestInfo = new TestSignalInfo(SamplesRateInSamplesPerSecond, freq);
            FreqIs2Hz = switchTo2Hz;
            EnablePowerRejector = true;
            if (FreqIs2Hz)
            {
                SetFilters(0, 20);
                foreach (var chanInfo in ChannelsInfo)
                {
                    chanInfo.SelectedHighFreq = 250.0f;
                    chanInfo.SelectedLowFreq = 0.5f;
                }
            }
            else
            {
                //SetFilters(200, 20000);
                SetFilters(1000, 20000);
                foreach (var chanInfo in ChannelsInfo)
                {
                    chanInfo.SelectedHighFreq = 10000.0f;
                    chanInfo.SelectedLowFreq = 160.0f;
                }
            }
            SetStandParams(freq);
            ResetDataStatistics();
        ...
        {
            MepDevice.SetFiltersHi(maxFreq);
            MepDevice.SetFiltersLo(minFreq);
        ...
        protected override void OnReadSamplesStimulsData(ReceiveDataArgs e, object sender)
        {
            ...
            //    for (int i = 0; i < ChannelsCount; i++)
            //    {
            //        NeuroSoft.MathLib.Basic.MulConst_IPP(e.Data[i], 0.8f);
            //    }
            //}
    ...
    public class InputResistanceInfo : DATBaseViewModel
    {
        ...
        internal void Reset()
        {
            ...
            Resistance = new RangedValue<double>(double.NaN, 9e+7, 1e+12);
            OnPropertyChanged("Resistance");
            OnPropertyChanged("ResistanceIsValid");
            Capacity = new RangedValue<double>(double.NaN, 1e-12, 30e-12);
            OnPropertyChanged("Capacity");
